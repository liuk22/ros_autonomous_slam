...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        start, goal = (
            convert_point(robot_location, [192, 192], 0),
            convert_point([2, 0.5], [192, 192], 0),
        )
        print(start, goal)
        if flag:
            path, graph = find_path_RRT(
                convert_point(robot_location, [192, 192], 0),
                convert_point([-1, 0.5], [192, 192], 0),
                cv2.cvtColor(map_img(current_map), cv2.COLOR_GRAY2BGR)[::-1],
            )
            print(path)
            print(graph)
            flag = False
