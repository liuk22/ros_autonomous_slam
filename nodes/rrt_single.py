...
def addToGraph(ax, graph, newPoints, point):
    if len(newPoints) > 1:  # If there is anything to add to the graph
        for p in range(len(newPoints) - 1):
            nearest = [
                nearest
                for nearest in graph
                if (nearest[0] == [newPoints[p][0], newPoints[p][1]])
            ]
            ...
            if not p == 0:
                ax.plot(
                    newPoints[p][0], newPoints[p][1], "+k"
                )  # First point is already painted
            ax.plot(
                [newPoints[p][0], newPoints[p + 1][0]],
                [newPoints[p][1], newPoints[p + 1][1]],
                color="k",
                linestyle="-",
                linewidth=1,
            )
        ...
        if point in newPoints:
            ...
            ax.plot(
                newPoints[p + 1][0], newPoints[p + 1][1], "+k"
            )  # Last point is not green
...
def connectPoints(a, b, img):
    ...
    for i in range(pointsNeeded + 1):  # Creates points between graph and solitary point
        ...
        if not (coordX == a[0] and coordY == a[1]):
            newPoints.append(
                [
                    newPoints[i][0] + (step[0] * STEP_DISTANCE),
