...
def callback(data):
    ...
    for i in range(360):
        ...
        if x[i] == 0:
            ...
            # store scan data
    y_l = min(
        x[side_scanstartangle : side_scanstartangle + side_scanrange]
    )  # left wall distance
    y_r = min(
        x[360 - side_scanstartangle - side_scanrange : 360 - side_scanstartangle]
    )  # right wall distance
    s_d = min(
        min(x[0 : int(front_scanrange / 2)], x[int(360 - front_scanrange / 2) : 360])
    )  # front wall distance
